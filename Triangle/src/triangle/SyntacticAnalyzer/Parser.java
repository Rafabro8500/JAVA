...
// to represent its phrase structure.
  ...
  Command ifAuxCommand(SourcePosition commandPos)throws SyntaxError{
      ...
      Expression eAST = parseExpression();
        ...
        if (currentToken.kind == Token.ELSE){
            ...
            accept(Token.ELSIF);
            commandAST = new IfCommand(eAST, c1AST, ifAuxCommand(commandPos), commandPos);
        }
...
///////////////////////////////////////////////////////////////////////////////
  ...
  Declaration ProcFuncs() throws SyntaxError{
      Declaration declarationAST = null; // in case there's a syntactic error
      SourcePosition declarationPos = new SourcePosition();
      start(declarationPos);
      declarationAST = ProcFunc();
      while(currentToken.kind == Token.AND){
          acceptIt();
          declarationAST  = new SequentialDeclaration(declarationAST, ProcFunc(),
        declarationPos);
      }
      return declarationAST;
  }
  ...
  Declaration compoundDeclaration()throws SyntaxError{
    ...
    if (currentToken.kind == Token.REC) {
        acceptIt();
        declarationAST = ProcFuncs();
        accept(Token.END);
        finish(declarationPos);
    }
    else if(currentToken.kind == Token.PRIVATE){
        acceptIt();
        ...
        declarationPos);
            ...
            currentToken.kind == Token.PROC || currentToken.kind == Token.FUNC){
        declarationAST = parseSingleDeclaration();
    ...
    case Token.ARRAY:
      {
        ...
        if(currentToken.kind == Token.OF){
        ...
        else if(currentToken.kind == Token.DOTDOT){
            acceptIt();
            ...
            finish(typePos);
            typeAST = new ArrayTypeDenoter(ilAST, tAST, typePos);
            typeAST = new ArrayTypeDenoter(i2AST, typeAST, typePos);
        }
      }
      break;
